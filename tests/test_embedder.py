...
def test_embedder_generic(model, thresh=10):
    ...
    img = cv2.imread('tests/smallapple.jpg')
    small_angle = 1
    ...
    img2 = cv2.imread('tests/rock.jpg')
...
class TestModule(unittest.TestCase):
    def test_embedder_torch(self):
        from embedder_pytorch import MobileNetv2_Embedder
        return test_embedder_generic(MobileNetv2_Embedder)

    def test_embedder_tf(self):
        from embedder_tf import MobileNetv2_Embedder
        return test_embedder_generic(MobileNetv2_Embedder)
